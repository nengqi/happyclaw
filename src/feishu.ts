...
export function createFeishuConnection(
  ...
  async function handleIncomingMessage(
    ...
    // Multi-tenant chat 白名单：只监听指定群聊/私聊。
    // shared bot 常在多个团队群里，不加白名单会响应所有 chat（污染）。
    // 空 env = 不限制（flag-off 安全，与 upstream 原行为一致）。
    ...
    // Phase 1：只路由 P2P；群消息走下方 else 维持原 onNewChat（群 sender 重绑留 phase 2）。
    if (onSenderRoute && chatType === 'p2p') {
      if (!senderOpenId) {
        logger.debug(
          { chatJid, messageId },
          'Multi-tenant: missing senderOpenId, dropping',
        );
        return;
      }
